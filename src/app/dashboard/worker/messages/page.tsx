...
export default async function MessagesPage({
  ...
  const threads = await getWorkerMessageThreads(user.id);

  if (threads.length === 0) {
    return (
      <div>
        <PageTitle>Messages</PageTitle>
        <EmptyState title="No conversations yet" hint="Chats open once you're hired — they're scoped to each hire." />
      </div>
    );
  }

  const activeId = thread ?? threads[0].hireId;
  const active = await getWorkerThread(activeId, user.id);
      ...
      <Card className="grid min-h-[480px] grid-cols-1 overflow-hidden p-0 md:grid-cols-[300px_1fr]">
        ...
        <div className="border-b border-line md:border-b-0 md:border-r">
          {threads.map((t) => (
            <Link
              key={t.hireId}
              href={`/dashboard/worker/messages?thread=${t.hireId}`}
          ...
          {active && (
            <>
              <div className="flex items-center justify-between border-b border-line px-6 py-3.5">
                <div>
                  <span className="text-sm font-semibold text-ink">{active.client.name}</span>
              ...
              <div className="flex flex-1 flex-col gap-3 p-6">
                {active.messages.map((m) => {
                  ...
                  return (
                    <div
                      key={m.id}
                      className={`max-w-[70%] rounded-2xl border px-4 py-2.5 ${
                        mine ? "self-end border-bronze/30 bg-bronze/[0.08]" : "self-start border-line bg-bg"
                      }`}
                    >
